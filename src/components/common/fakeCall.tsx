...
export default function FakeCall({ onClose, settings }: { onClose: () => void;
    settings: {
        name: string;
        photo: string;
        ringtone: string;
      ...
      {!answered ? (
        <>
          <div className="text-center">
            ...
            <h2 className="text-4xl font-bold mb-2">{settings.name || "Mom"}</h2>
            <img
          ...
          <div className="flex justify-center space-x-16 mt-6">
            <button onClick={onClose} className="bg-red-600 p-5 rounded-full">
              <PhoneOff className="w-6 h-6" />
            </button>
            <button onClick={() => setAnswered(true)} className="bg-green-600 p-5 rounded-full">
        ...
        <>
          <h2 className="text-2xl font-semibold mb-1">{settings.name || "Mom" }</h2>
          ...
          <button onClick={onClose} className="mt-6 bg-red-600 px-4 py-2 rounded-xl">
            End Call
          </button>
        </>
      )}
